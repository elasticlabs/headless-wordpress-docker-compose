...
if [ -f "./backup/mysql.sql.gz" ]; then
    ...
    zcat "backup/mysql.sql.gz" | docker exec -i $(docker-compose ps -q mariadb) mysql -u root -ppassword wordpress
    echo "FLUSH PRIVILEGES;" | docker exec -i $(docker-compose ps -q mariadb) mysql -u root -ppassword
