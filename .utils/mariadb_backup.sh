...
if [ "$backupOK" = "Yes" ] || [ "$backupOK" = "yes" ] || [ "$backupOK" = "Y" ] || [ "$backupOK" = "y" ]; then
    ...
    mkdir -p backup
    docker exec -i $(docker-compose ps -q mariadb) mysqldump -u root -ppassword --opt --single-transaction --events --databases wordpress --routines --comments | gzip > "backup/mysql.sql.gz"
